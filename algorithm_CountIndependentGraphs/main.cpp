...
int main() {
	...
	for (char fileCounter = '1'; fileCounter <= '5'; ++fileCounter) {
		...
		//adding the islands without roads to their only one piece city
		for (int index = 0; index < cities; ++index) {
			if (a[index] == 0) {
				++islands;
			}
		...
		std::cout << fileCounter << ":  szigetek szama: " << std::setw(5) << islands;
		std::cout << "    (futasi ido:" << std::setw(6) << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - startTime).count() << " millisec)" << std::endl;
	}
}